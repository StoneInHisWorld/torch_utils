...
def ResNet50(in_channels, num_layers=None, width=64, num_classes=1000):
    """
    ResNet50标准实现
...
class ResNet(BasicNN):
    ...
    def __init__(self,
        ...
        resnet_stages = nn.Sequential(OrderedDict([
            ('stage0', self._make_layer(block, width, num_layers[0], stride=1))
        ] + [
            (f'stage{i}', self._make_layer(block, width * 2 ** i, nl, stride=2))
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        self.in_channels = planes * block.expansion  # 更新输入通道数
