...
class Pix2Pix(BasicNN):
   ...
   """
    ...
    #     if o_args is None:
    #         o_args = [()]
    #     super(Pix2Pix, self).prepare_training(o_args, l_args, tr_ls_args, ts_ls_args)
    #     self.optimizer_s = self.netG.optimizer_s + self.netD.optimizer_s
    #     self.lr_names = self.netG.lr_names + self.netD.lr_names
    #     self.scheduler_s = self.netG.scheduler_s + self.netD.scheduler_s
    #     self.netG.train_ls_fn_s[0] = functools.partial(self.netG.train_ls_fn_s[0], netD=self.netD)
    #     self.netG.test_ls_fn_s[0] = functools.partial(self.netG.test_ls_fn_s[0], netD=self.netD)
    #     self.train_ls_names = self.netG.train_ls_names + self.netD.train_ls_names
    #     self.test_ls_names = self.netG.test_ls_names + self.netD.test_ls_names
