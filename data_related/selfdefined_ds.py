...
class SelfDefinedDataSet:
    ...
    def __init__(self, module: type, config: dict or ControlPanel,
        ...
        if is_train:
            ...
            # indexes = (None if self._f_lazy else self._train_f,
            #            None if self._l_lazy else self._train_l)
            # # if self._f_lazy:
            # #     print('对于训练集，实行懒加载特征数据集')
            # # else:
            ...
            # self._train_f, self._train_l = self.reader.fetch(*indexes, not self.bulk_preprocess)
        ...
        #         n_workers=self.n_workers
        #     )
        # indexes = (None if self._f_lazy else self._test_f,
        #            None if self._l_lazy else self._test_l)
        # self._test_f, self._test_l = self.reader.fetch(*indexes, not self.bulk_preprocess)
